...
var _DocType_HBL = "'CIPL', 'HBL', 'CO', 'INV', 'CC', 'IP','HDC'";
...
$(document).on("click", "a[name='layer_Tracking_btn']", function () {    
    _vHblNo = $(this).siblings("input").val();    
    fnGetTracking(_vHblNo, "");

    //fnGetTrackLocation($(this).siblings("input").val());
});

//Pre-alert 버튼 클릭 이벤트 (문서 관련 데이터 가져와서 뿌려주기)
//$(document).on("click", "a[name='file_layer']", function (e) {
//    try {
//        $(this).siblings(".file_layer").addClass("show");
//        //var vHTML = "";
//        ////vHTML = fnGetPreAlert($(this).siblings("input[type='hidden']").val());
//        //vHTML = fnGetPreAlert($(this).siblings("input[type='hidden']").val());
//        //if (vHTML == "N") {
...
//        //}
//        //else {
//        //    $(this).siblings(".file_layer").find("ul")[0].innerHTML = vHTML;
...
$(document).on("click", ".file_view", function (e) {
    ...
    try {
        if ($(this).find("ul").hasClass("file_nodata")) {
            ...
            //초기화
            $(".file_layer_new").removeClass("show");
            //$(this).find(".file_layer").addClass("show");
        }
        
        //if ($(this).find(".file_nodata")) {
        //    _fnAlertMsg("해당 B/L의 문서가 없습니다.");
        //} else {
        //    $(this).find(".file_layer").addClass("show");        
        //}
...
function fnBLValidation() {
    ...
    //    _fnAlertMsg("출발지를 선택해주세요.","input_Departure");
    ...
    //    _fnAlertMsg("도착지를 선택해주세요.","input_Arrival");
    //    $("#input_Arrival").focus();
    //    return false;
    //}
...
function fnBLPrint(vHBL, vDocType) {
    ...
    try {
        ...
        var vResult;
        vResult = fnGetBLPrintData(vHBL, vDocType); //데이터 가져오기
                            ...
                            if (file_nm.substring(file_nm.length, file_nm.length - 4) == ".pdf" || file_nm.substring(file_nm.length, file_nm.length - 4) == ".PDF") {
                                if (agent.indexOf('trident') > -1) {
                                    ...
                                    if (vDocType == "CHBL" || (vDocType == "HBL" && _vREQ_SVC == "AIR")) {
                                        $("#layer_Request_area").val(""); //twkim - 다시 수정요청사항을 켰을 때 Textarea 초기화
...
function fnSetSearchData(vJsonData) {
    if (_fnToNull(JSON.parse(vJsonData).Result[0]["trxCode"]) == "Y") {
        ...
        $("#input_ETD").val(_fnFormatDate(_fnToNull(JSON.parse(vJsonData).BL[0]["ETD"].replace(/\./gi, "")))); //ETD
        $("#input_ETA").val(_fnFormatDate(_fnToNull(JSON.parse(vJsonData).BL[0]["ETA"].replace(/\./gi, "")))); //ETA

        $("#input_Departure").val(_fnToNull(JSON.parse(vJsonData).BL[0]["POL_NM"])); //POL_NM
        $("#input_POL").val(_fnToNull(JSON.parse(vJsonData).BL[0]["POL_CD"])); //POL_CD
        $("#input_Arrival").val(_fnToNull(JSON.parse(vJsonData).BL[0]["POD_NM"])); //POD_NM
        $("#input_POD").val(_fnToNull(JSON.parse(vJsonData).BL[0]["POD_CD"])); //POD_CD
        ...
        if (JSON.parse(vJsonData).Result[0]["trxCode"] == "Y") {
            ...
            if (vResult.length > 0) {
                $.each(vResult, function (i) {
                    ...
                    //문서파일 데이터가져오기
                    vHTML_DocList = "";
                    vHTML_DocList = fnGetDocFile(_fnToNull(vResult[i]["HBL_NO"]));
                    //if (_fnToNull(vResult[i]["HBL_NO"]) != "" && $("#Session_USR_TYPE").val() == "S") {
                    //    vHTML_DocList = "";
                    //    vHTML_DocList = fnGetDocFile(_fnToNull(vResult[i]["HBL_NO"]));
                    //}
                    ...
                    if (_fnToNull(vResult[i]["REQ_SVC"]) == "SEA") {
                        ...
                        //    vHTML += "      <div class=\"btn_padding file_view\" name=\"layer_file_view\"> ";
                        //    vHTML += "      	<span class=\"btn_type1 orange file\" name=\"file_layer\">Doc</span> ";
                        //    vHTML += "      	<div class=\"file_layer file_layer" + i + "\"> ";
                        //    vHTML += "      		<div class=\"scrollbar_file\"> ";
                        //    if (vHTML_DocList == "N") {
                        //        vHTML += "      		<ul class=\"file_list file_nodata\"> ";
                        //    } else if (vHTML_DocList == "E") {
                        ...
                        //    vHTML += "   </td> ";
                        //}
                        //else if ($("#Session_USR_TYPE").val() == "P") { //twkim - 파트너
                        //    vHTML += "   <td class=\"btns_w1\">	 ";
                        //    vHTML += "      <div class=\"btn_padding file_view\" name=\"layer_file_view\"> ";
                    ...
                    //항공
                        ...
                        //    vHTML += "   	<div class=\"btn_padding\"> ";
                        //    vHTML += "   		<a href=\"javascript:void(0)\" class=\"btn_type1 skyblue\" name=\"layer_Tracking_btn\">Tracking</a> ";
                        //    vHTML += "          <input type=\"hidden\" value=\"" + _fnToNull(vResult[i]["VSL"]) + "\" /> ";
                        //    vHTML += "   	</div> ";
                        //
                        //    //twkim - 화주 문서 등록
                        //    vHTML += "      <div class=\"btn_padding file_view\" name=\"layer_file_view\"> ";
...
function fnMakeBLRequest(vJsonData) {
    ...
    if (JSON.parse(vJsonData).Result[0]["trxCode"] == "Y") {
        ...
        //    if (RMK[0]["SEQ"] == 1) {
        //        $("#RMKREAD").show();
        //        $("#RMKREAD").text(RMK[0]["SEQ"] + "차 요청사항 입니다\n" + RMK[0]["RMK"] + "\n===============\n" + (RMK[0]["SEQ"] + 1) + "차 요청사항입니다 \n");
        //        $("#RMK").val("");
        //    } else {
        //        $("#RMKREAD").show();
        //        $("#RMKREAD").text(RMK[0]["RMK"] + "\n===============\n" + (RMK[0]["SEQ"] + 1) + "차 요청사항입니다\n");
        //        $("#RMK").val("");
        //    }
        //} else {
        //    $("#RMKREAD").hide();
...
function fnMakeDocList(vJsonData) {
    ...
    if (JSON.parse(vJsonData).Result[0]["trxCode"] == "Y") {
        ...
        //HDC가 잇는지 없는지 확인하는 로직 필요
        
        $.each(vResult, function (i) {
            vHTML += "   <li> ";
            vHTML += "   	<div class=\"file_sort\"><p>" + _fnToNull(vResult[i]["DOC_NM"]) + "</p></div> ";
            vHTML += "      <div class=\"file_nm\"> ";
            vHTML += "      	<a href=\"javascript:void(0)\" class=\"file_name btnDocDownFile\">" + vResult[i]["MNGT_NO"] + "</a> ";
...
function fnMakePreAlertList(vJsonData) {
    ...
    if (JSON.parse(vJsonData).Result[0]["trxCode"] == "Y") {
        ...
        //HDC가 잇는지 없는지 확인하는 로직 필요 (하우스 HOUSE DC가 있으면 마스터 DC는 필요가 없다)
        $.each(vResult, function (i) {
