...
$(document).on("click", ".list_type1.bk_list tr", function () {
    ...
    if ($(this).closest('tr').find("td").eq(0).text() == "Y") {
        ...
        if (!Tr.hasClass("hold")) {
            ...
            //twkim - mobile용 선택
            $('.list_type1.bk_list .layout_type2').removeClass("hold");
            Tr.find(".layout_type2").addClass("hold");
            ...
            //twkim - mobile일 경우 이동하는 로직이 필요 할 것 같다.
            if (matchMedia("screen and (max-width: 1024px)").matches) {
                //현재 선택한 tr을 스크롤 위에다가 붙히기 - 하는 중 아직 완성 X
                //alert(Tr.find(".layout_type2").offset().top - 507);
                //$(".scrollbar_booking").animate({ scrollTop: Tr.find(".layout_type2").offset().top - 507 }, 0);
                fnMovePage("booking_status_title");
            }
...
function fnGetTariff() {
    ...
    try {
        ...
        if (_vREQ_SVC == "SEA") {
            ...
            //twkim 20220307 - 담당자의 요청으로 해운같은경우 무조건 별도 문의로 지정
            vHTML = "별도문의";
            ...
            if (vResult.length > 0) {
                ...
                $.each(vResult, function (i) {
                    ...
                    if (_fnToNull(vResult[i]["ETD_HM"]) != "") {
                        vHTML += "<br />" + _fnFormatHHMMTime(_fnToNull(vResult[i]["ETD_HM"]));
                    }
                    vHTML += "   	</td> ";
                    vHTML += "   	<td> ";
                    ...
                    if (_fnToNull(vResult[i]["ETA_HM"]) != "") {
                        vHTML += "<br />" + _fnFormatHHMMTime(_fnToNull(vResult[i]["ETA_HM"]));
                    }
                    vHTML += "   	</td> ";
...
function fnMakeAirTariff(vJsonData) {
    ...
    try {
        ...
        if (JSON.parse(vJsonData).Result[0]["trxCode"] == "Y") {
            ...
            //Validation Check 1. AAF , AFS 중 하나라도 없을 경우 , 2. 화폐 단위가 안되는 경우 
            if (vResult.length != 2) {
                vHTML = "별도문의";
                $("#booking_detail_tariff_PRC").val(0);
                $("#booking_detail_tariff_CURR_CD").val("USD");
            }
            else if (_fnToNull(vResult[0]["CTRT_CURR_CD"]) != _fnToNull(vResult[1]["CTRT_CURR_CD"])) {
                vHTML = "별도문의";
                $("#booking_detail_tariff_PRC").val(0);
                $("#booking_detail_tariff_CURR_CD").val("USD");
            }
                ...
                //2개 합쳐서 데이터 가져오기
                var vTotal = (Number(_fnToNull(vResult[0]["UNIT_PRC"])) * Number(_fnToNull(vResult[0]["CHB_WGT"])) + Number(_fnToNull(vResult[1]["UNIT_PRC"])) * Number(_fnToNull(vResult[1]["CHB_WGT"])));

                if (_fnToNull(vResult[0]["CTRT_CURR_CD"]) == "USD") {
                    vHTML = _fnToNull(vResult[0]["CTRT_CURR_CD"]) + " " + vTotal.toFixed(2);
                } else if (_fnToNull(vResult[0]["CTRT_CURR_CD"]) == "KRW") {
                    vHTML = _fnToNull(vResult[0]["CTRT_CURR_CD"]) + " " + fnSetComma(vTotal);
                }
