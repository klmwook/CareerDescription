...
function goView(pageID) {
    location.href = "/Recruitment/Job_vacancyView?id=" + pageID;
}
...
function _fnSearchData(pageIndex) {
        ...
        if (rtnVal == undefined) {
            ...
            $(rtnVal).each(function (i) {
                ...
                if (rtnVal[i].NOTICE_YN == "y") {
                    ...
                    innerText += "	<td class='a_left'><a href='#' onclick='goView(" + rtnVal[i].RECRUITMENT_ID + ")'>" + rtnVal[i].TITLE + "</a></td> ";
    ...
    else
    {
        if (rtnVal == undefined) {
            ...
            innerText += "<tbody> ";
            innerText += "<tr> ";
            innerText += "<td colspan=\"4\">등록된 데이터가 없습니다</td>";
            innerText += "</tr> ";
            innerText += "</tbody> ";
            innerText += "</table> ";
        } else {
