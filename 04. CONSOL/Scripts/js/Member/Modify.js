            ...
            $.ajax({
                ...
                success: function (result) {
                    if (JSON.parse(result).Result[0]["trxCode"] == "Y") {
                        ...
                        alert("회원 정보가 수정 되지 않았습니다. 담당자에게 문의 하세요.");
                    } else if (JSON.parse(result).Result[0]["trxCode"] == "E") {
                        alert("담당자에게 문의 하세요.");                        
                    }
                }, error: function (xhr, status, error) {
...
function fnModifySave_Validation() {
    try {        
        ...
        if (_chkpwd) {
            alert("현재 비밀번호가 틀립니다. 다시 확인해주시기 바랍니다.");
            $("#RES_NOWPWD").focus();
            return false;
        }
        ...
        if (_fnToNull(vNewPwd) != "" || _fnToNull(vCheckNewPwd) != "") {
            ...
            if (vNewPwd != vCheckNewPwd) {
                alert("새 비밀번호와 확인 비밀번호가 같지 않습니다.");
                return false;
            }
            if (vNewPwd.length < 6) {
                alert("비밀번호의 자리수는 6자리 이상 16자리 이하로 입력 해 주세요.");
                return false;
            }
            if (!vCheck.test(vNewPwd)) {
                alert("비밀번호는 영문,숫자,특수문자를 포함하여 입력 해 주세요.");
                return false;
            }
        }
