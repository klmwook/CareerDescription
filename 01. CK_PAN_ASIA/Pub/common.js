...
$(function () {
	...
	//twkim - CK의 바로가기 2초 있다가 사라지게 끔 수정
	setTimeout(function () {
		$(".navi_tab ul").removeClass("on");
	}, 2000);
	...
	$('.navi_tab .navi_list').on("click", function () {
		...
		//화물진행 , 수출이행이 4,5번째 여서 +3로 추가한다. (선사 스케줄 , 항공 스케줄 , 화물진행 , 수출이행)
		var vIndex = $(this).index() + 3;
	...
	input.focus(function () {
		if ($(this).siblings('.btns.icon').hasClass('down')) {
			$(this).siblings('.btns.icon.down').css('background', 'transparent url(/Images/icn_map_c.png) no-repeat 50% 50%');
		}
		else if ($(this).siblings('.btns.icon').hasClass('calendar')) {
			$(this).siblings('.btns.icon.calendar').css('background', 'transparent url(/Images/icn_calendar_c.png) no-repeat 50% 50%');
		}
	});
	input.blur(function () {
		if ($(this).siblings('.btns.icon').hasClass('down')) {
			$(this).siblings('.btns.icon.down').css('background', 'transparent url(/Images/icn_map.png) no-repeat 50% 50%');
		}
		else if ($(this).siblings('.btns.icon').hasClass('calendar')) {
			$(this).siblings('.btns.icon.calendar').css('background', 'transparent url(/Images/icn_calendar.png) no-repeat 50% 50%');
		}
	})
...
function fnMovePage(vId) {
	...
	if (windowWidth < 1025) {
		...
		$('html, body').animate({ scrollTop: offset.top - 91 }, 350);
	}
...
function fnInitClickTab(vTab) {	
	...
	else if (vTab.attr("id") == "seaSchedule")
	{
		$("table[name='Main_SEATable_List'] th button").removeClass();
		$("#SEA_Schedule_AREA").hide();
		$("#Btn_SEAScheduleMore").hide();
