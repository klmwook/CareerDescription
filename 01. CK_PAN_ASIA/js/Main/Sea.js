...
window.onpageshow = function (event) {
	if (event.persisted || (window.performance && window.performance.navigation.type == 2) || event.originalEvent && event.originalEvent.persisted) {
		if (_fnToNull(sessionStorage.getItem("LINE_TYPE")) == "SEA" && _fnToNull(sessionStorage.getItem("BEFORE_VIEW_NAME")) == "MAIN_SEA") {
			...
			$("#select_SEA_CntrType").val(_fnToNull(sessionStorage.getItem("CNTR_TYPE"))); 
			...
			//TWKIM 20230104 - 엑스 박스 표시 유무 추가
			$("#input_SEA_Departure").val().length > 0 ? $("#input_SEA_Departure").siblings(".delete").show() : $("#input_SEA_Departure").siblings(".delete").hide();
			$("#input_SEA_Arrival").val().length > 0 ? $("#input_SEA_Arrival").siblings(".delete").show() : $("#input_SEA_Arrival").siblings(".delete").hide();
			...
			//TWKIM 20230104 - 엑스 박스 표시 유무 추가
			$("#input_AIR_Departure").val().length > 0 ? $("#input_AIR_Departure").siblings(".delete").show() : $("#input_AIR_Departure").siblings(".delete").hide();
			$("#input_AIR_Arrival").val().length > 0 ? $("#input_AIR_Arrival").siblings(".delete").show() : $("#input_AIR_Arrival").siblings(".delete").hide();
			sessionStorage.clear();
			$("#airSchedule").show();
			fnGetAIRScheduleData();
		} else if (_fnToNull(sessionStorage.getItem("LINE_TYPE")) == "SEA" && _fnToNull(sessionStorage.getItem("BEFORE_VIEW_NAME")) == "MAIN_LCL") {
			$(".panel").hide();
			$(".tab li").removeClass('on');
			$(".tab li[value=LCL]").addClass('on');
			$("#select_LCL_Bound").val(_fnToNull(sessionStorage.getItem("LCL_BOUND"))); //LCL_BOUND
			$("#input_LCL_ETD").val(_fnToNull(sessionStorage.getItem("ETD"))); //ETD
			$("#input_LCL_Departure").val(_fnToNull(sessionStorage.getItem("POL_NM"))); //
			$("#input_LCL_POL").val(_fnToNull(sessionStorage.getItem("POL_CD"))); //POL_CD
			$("#input_LCL_Arrival").val(_fnToNull(sessionStorage.getItem("POD_NM"))); //POD_NM
			$("#input_LCL_POD").val(_fnToNull(sessionStorage.getItem("POD_CD"))); //POD_CD

			//TWKIM 20230104 - 엑스 박스 표시 유무 추가
			$("#input_LCL_Departure").val().length > 0 ? $("#input_LCL_Departure").siblings(".delete").show() : $("#input_LCL_Departure").siblings(".delete").hide();
			$("#input_LCL_Arrival").val().length > 0 ? $("#input_LCL_Arrival").siblings(".delete").show() : $("#input_LCL_Arrival").siblings(".delete").hide();
...
function fnSEACheckCarr() {
	...
	if (_SEAObjCheck.CNTR_TYPE != $("#select_SEA_CntrType").find("option:selected").val()) {
		vCheck = false;
	}
		...
		if (vResult == undefined) {
			...
			if (vResult.length > 0) {
				...
				$.each(vResult, function (i) {
					...
					vHTML += "   	</td> ";
					vHTML += "   	<td> ";
					vHTML += _fnCK_DateFormat(_fnToNull(vResult[i]["ETD"]));
					vHTML += "   	</td> ";
					vHTML += "   	<td> ";
					vHTML += _fnCK_DateFormat(_fnToNull(vResult[i]["ETA"]));
					vHTML += "   	</td> ";
					...
					if (_fnToNull(vResult[i]["DOC_CLOSE_YMD"]) == "" || _fnToNull(vResult[i]["DOC_CLOSE_YMD"]) == "0") {
						...
						vHTML += _fnCK_DateFormat(_fnToNull(vResult[i]["DOC_CLOSE_YMD"]))+" ";						
					...
					if (_fnToNull($("#Session_USR_TYPE").val()) == "P") {
						vHTML += "			<a href=\"javascript:void(0)\"  class=\"btn_type1 gray\" style='cursor:default'>견적 확인</a>";
					} else {
						if (_fnToNull(vResult[i]["DOC_CLOSE_YMD"]) + _fnFormatHHMMTime(_fnToNull(vResult[i]["DOC_CLOSE_HM"])).replace(/:/gi, "") < _fnGetDateStamp()) {
							vHTML += "			<a href=\"javascript:void(0)\" name=\"btn_SEA_Booking_Close\" class=\"btn_type1 btnClose\">견적 확인</a>";
						} else {
							vHTML += "			<a href=\"javascript:void(0)\" name=\"btn_SEA_Booking\" class=\"btn_type1\">견적 확인</a>";
						}
					}
					...
					vHTML += "   						<td>" + _fnCK_DateFormat(_fnToNull(vResult[i]["ETD"])) + "</td> ";
					vHTML += "   					</tr> ";
					vHTML += "   					<tr> ";
					vHTML += "   						<th>Arrival</th> ";
					vHTML += "   						<td>" + _fnCK_DateFormat(_fnToNull(vResult[i]["ETA"])) + "</td> ";
					vHTML += "   					</tr> ";
					vHTML += "   					<tr> ";
