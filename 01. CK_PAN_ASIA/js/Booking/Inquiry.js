...
$(function () {
    ...
    if (event.persisted || (window.performance && window.performance.navigation.type == 2) || event.originalEvent && event.originalEvent.persisted) {
        if (_fnToNull(sessionStorage.getItem("BEFORE_VIEW_NAME")) == "BOOKING_BKG") {
            ...
            _vREQ_SVC = sessionStorage.getItem("REQ_SVC");
            if (_vREQ_SVC == "SEA") {
                $("#date_interval01").prop('checked', true);
                $("#date_interval03").prop('checked', false);
            }
            else if (_vREQ_SVC == "AIR") {
                $("#date_interval01").prop('checked', false);
                $("#date_interval03").prop('checked', true);
            }
            ...
            $("#input_Departure").val().length > 0 ? $("#input_Departure").siblings(".delete").show() : $("#input_Departure").siblings(".delete").hide();
            $("#input_Arrival").val().length > 0 ? $("#input_Arrival").siblings(".delete").show() : $("#input_Arrival").siblings(".delete").hide();
            $("#select_BKDetail_BkgNo").val().length > 0 ? $("#select_BKDetail_BkgNo").siblings(".delete").show() : $("#select_BKDetail_BkgNo").siblings(".delete").hide();
        ...
        if (JSON.parse(vJsonData).Result[0]["trxCode"] == "Y") {
            ...
            if (vResult == undefined) {
                ...
                $.each(vResult, function (i) {
                    ...
                    vHTML += "   	<td> ";
                    if (_fnToNull(vResult[i]["LINE_CD"]) != "") {
