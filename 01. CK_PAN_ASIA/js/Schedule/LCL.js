...
$(function () {
	...
	if (_fnToNull($("#Session_USR_ID").val()) == "") {
		...
		if (event.persisted || (window.performance && window.performance.navigation.type == 2) || event.originalEvent && event.originalEvent.persisted) {
			if (_fnToNull(sessionStorage.getItem("BEFORE_VIEW_NAME")) == "SCHEDULE_LCL") {
				...
				//검색
				$("#btn_LCLSchedule_Search").click();
			}
		}

		$("#input_LCL_ETD").val(_fnPlusDate(0)); //ETD
		fnGetLCLBound();
    }
...
function fnLCLSetBooking(vThis) {
	try {
		...
		sessionStorage.setItem("BEFORE_VIEW_NAME", "SCHEDULE_LCL");
