...
function fnSetSection() {
	...
	try {
		...
		objJsonData.PERIOD_YEAR = _fnToNull(vYear_Quarter[0]); //년
		objJsonData.PERIOD_QUARTER = _fnToNull(vYear_Quarter[1]); //분기
...
function fnTariffSerach() {
	...
	objJsonData.YEAR = _fnToNull(vYear_Quarter[0]); //년
	objJsonData.QUARTER = _fnToNull(vYear_Quarter[1]); //분기
	...
	//TWKIM - 변경
	objJsonData.PORT = _fnToNull(vSECTION[0]); //PORT
	objJsonData.SECTION = _fnToNull(vSECTION[1]); //편도 , 왕복
...
function fnMakeTariffData(vJsonData) {
	...
	if (JSON.parse(vJsonData).Result[0]["trxCode"] == "Y") {
		...
		vHTML += " <span>육상운송료 담당자를 통해 안내가 필요한 구간입니다. <br/> ";
		vHTML += " 아래 담당자를 찾아주시면 답변 드리겠습니다.  <br/> <br/> ";
		vHTML += " 담당자 : 웰컴매니저 이지홍 <br> ";
		vHTML += " 전화 : 02-712-7200<br> ";
		//vHTML += " LCL 담당자 : 웰컴 매니저 임   춘<br> ";
		vHTML += " 메일 : seoul@ckpanasia.com</span> ";
