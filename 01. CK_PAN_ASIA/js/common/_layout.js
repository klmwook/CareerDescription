...
$(function () {
    ...
    var userInputId = _fnGetCookie("Prime_CK_USR_ID_REMEMBER_CKPA");
    var vAutoLogin = _fnGetCookie("Prime_CK_PASSWORD_REMEMBER_CKPA");
...
$(document).on("keyup", "#Pc_btn_Tracking_Layer", function (e) {
    if (e.keyCode == 13) {
        if (_fnToNull($("#Pc_Input_Tracking_Layer").val()) != "") {
            fngetLayerTracking($("#Pc_Input_Tracking_Layer").val().toUpperCase().trim(), "");
        } else {
...
$(document).on("click", "#L_btnAirTrkSearch", function () {
    fngetLayerTracking($("#Pc_Input_Tracking_Layer").val().toUpperCase().trim(), $("#Pc_Input_Tracking_Layer").val().toUpperCase().trim());
});
    ...
    try {
        ...
        $.ajax({
            ...
            beforeSend: function (xhr) {
                xhr.setRequestHeader('Authorization', _objTrkLayer.TOKEN);
            },
            type: "GET",
            async: false,
            dataType: "json",
...
function fnMakeLayerTrkList(vJsonData) {
    try {
        var vHTML = "";
        var vResult = "";
        ...
        if (JSON.parse(vJsonData).Result[0]["trxCode"] == "Y") {
            ...
            if (_fnToNull(vResult) != "") {
                ...
                $("#Pc_Input_Tracking_Layer").val(_fnToNull(vResult[0].HBL_NO));
                ...
                if (vResult[i].REQ_SVC == "SEA") {
                    if (vResult[i].EX_IM_TYPE == "E") {
                        ...
                        if (_fnToNull(vResult[i].EVENT_STATUS) == "Y") {
                            vHTML += "   <div class=\"track_stat on " + vLast + " export\"> ";
                            checkLast = true;
                        }
                        else if (_fnToNull(vResult[i].EVENT_STATUS) == "N") {
                            vHTML += "   <div class=\"track_stat yet " + vLast + " export\"> ";
                        }
                        else if (_fnToNull(vResult[i].EVENT_STATUS) == "E") {
                            vHTML += "   <div class=\"track_stat " + vLast + " export\"> ";
                        }
                        ...
                        if (_fnToNull(vResult[i].EVENT_STATUS) == "Y") {
                            vHTML += "   <div class=\"track_stat on " + vLast + " import\"> ";
                            checkLast = true;
                        }
                        else if (_fnToNull(vResult[i].EVENT_STATUS) == "N") {
                            vHTML += "   <div class=\"track_stat yet " + vLast + " import\"> ";
                            checkLast = false;
                        }
                        else if (_fnToNull(vResult[i].EVENT_STATUS) == "E") {
                            vHTML += "   <div class=\"track_stat " + vLast + " import\"> ";
                            checkLast = false;
                        }
            ...
            success: function (result) {
                ...
                if (JSON.parse(result).Result[0]["trxCode"] == "Y") {
                    ...
                    if (JSON.parse(result).Table[0].APV_YN == "Y") {
                        ...
                        //자동 로그인일 경우
                        if ($('input[name=auto_login_keep]')[0].checked) {
                            _fnSetCookie("Prime_CK_USR_ID_REMEMBER_CKPA", JSON.parse(result).Table[0].USR_ID, "168");
                            _fnSetCookie("Prime_CK_PASSWORD_REMEMBER_CKPA", $("#Login_Password").val(), "168");
                        }
                        else {
                            _fnDelCookie("Prime_CK_USR_ID_REMEMBER_CKPA");
                            _fnDelCookie("Prime_CK_PASSWORD_REMEMBER_CKPA");
                        ...
                        $.ajax({
                            ...
                            success: function (result, status, xhr) {
                                ...
                                if (_fnToNull(result) == "Y") {
                                    ...
                                    else if (_initPage.split(';')[0] == "goLCLBooking") {
                                        var objJsonData = new Object();
                                        objJsonData.SCH_NO = _initPage.split(';')[1];
                                        objJsonData.SEQ = _initPage.split(';')[2];
                    ...
                    _fnDelCookie("Prime_CK_PASSWORD_REMEMBER_CKPA"); //자동 로그인 시 계속 뜰수가 있음. 실패하면 삭제.
                    if (Login_Count >= 0) {
...
function _fnLogout() {
    $.ajax({
        ...
        success: function (result, status, xhr) {
            ...
            _fnDelCookie("Prime_CK_PASSWORD_REMEMBER_CKPA"); //자동 로그인 떄문에 넣어둠
	...
	try {
        ...
        if (vPortPOL == vPortPOD) {
            ...
            if (vType == "Q") { //Quick 즐겨찾기 포트 
                _fnAlertMsg("출발지와 도착지의 Port가 동일합니다.", vInitCD);
                _QuickMenu = vPopID;                
                $("#" + vInit).val("");
                $("#" + vInitCD).val("");
            }
