...
$(document).on("click", ".layer_invoice_btn", function () {
    ...
    //alert($(this).siblings("input[name='HBL_NO']").val());
    //alert($(this).siblings("input[name='INV_NO']").val());
...
function fnSetSearchData(vJsonData) {
    if (_fnToNull(JSON.parse(vJsonData).Result[0]["trxCode"]) == "Y") {
        ...
        $("#input_ETD").val(_fnFormatDate(_fnToNull(JSON.parse(vJsonData).Invoice[0]["ETD"].replace(/\./gi, "")))); //ETD
        $("#input_ETA").val(_fnFormatDate(_fnToNull(JSON.parse(vJsonData).Invoice[0]["ETA"].replace(/\./gi, "")))); //ETA

        $("#input_Departure").val(_fnToNull(JSON.parse(vJsonData).Invoice[0]["POL_NM"])); //POL_NM
        $("#input_POL").val(_fnToNull(JSON.parse(vJsonData).Invoice[0]["POL_CD"])); //POL_CD
        $("#input_Arrival").val(_fnToNull(JSON.parse(vJsonData).Invoice[0]["POD_NM"])); //POD_NM
        $("#input_POD").val(_fnToNull(JSON.parse(vJsonData).Invoice[0]["POD_CD"])); //POD_CD
