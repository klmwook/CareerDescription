...
$(function () {
    ...
    if (_fnToNull($("#Session_USR_ID").val()) != "") {
        window.location = window.location.origin + "/L_Receipt";
        
    } else {
        var vUSR_ID = _fnGetCookie("Prime_CK_USR_ID_REMEMBER_DHL0");
        var vDOMAIN = _fnGetCookie("Prime_CK_USR_DOMAIN_REMEMBER_DHL0");
...
function fnLogin() {
    try {
        ...
        //도메인 세팅
        if (_fnToNull($("#Domain").val().replace(/ /gi, "")) == _LOLOPIANA_Domain) {
            objJsonData.DB_TYPE = "LOLOPIANA";
            objJsonData.DOMAIN = _LOLOPIANA_Domain;
        }
        else {
            ...
            success: function (result) {               
                ...
                if (JSON.parse(result).Result[0]["trxCode"] == "Y") {
                    ...
                    if ($("#login_keep").is(":checked")) {
                        _fnSetCookie("Prime_CK_USR_ID_REMEMBER_DHL0", objJsonData.USR_ID, "168");
                        _fnSetCookie("Prime_CK_USR_DOMAIN_REMEMBER_DHL0", objJsonData.DOMAIN, "168");
                    }
                    else {
                        _fnDelCookie("Prime_CK_USR_ID_REMEMBER_DHL0");
                        _fnDelCookie("Prime_CK_USR_DOMAIN_REMEMBER_DHL0");
                    }
