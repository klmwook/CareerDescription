            ...
            if (!vBoolean) {
                if ($(this).hasClass("selected")) {
                    ...
                    if ($(this).find(".EAN").text() == vEAN_CD) {
                        ...
                        if ($(this).find(".BAT").text() == vBATCH_ID) {
                            ...
                            if (fnSetPlanData(this, vBATCH_ID)) {
                                $("#Picking_Result_Area .selected").removeClass("selected"); //전체 삭제
                                $(this).addClass("on");
                                vBoolean = true;
                            } else {
...
//BAT가 맞지 않으면 데이터를 확인해야된다.
function fnSetPlanData(vThis, vBATCH_ID) {
    try {
        vResult = false;
        var objJsonData = new Object();
        ...
        objJsonData.BATCH_ID = vBATCH_ID;
        ...
        objJsonData.TRA_MNGT_NO = $("#Select_OrderID").find("option:selected").val();
        objJsonData.RCP_MNGT_NO = $(vThis).find("td").eq(1).text();
        objJsonData.RCP_BOX_ID = $(vThis).find("td").eq(2).text();
        objJsonData.RCP_ITEM_SEQ = $(vThis).find("td").eq(3).text();

        $.ajax({
            type: "POST",
            url: "/L_Picking/fnSetPlanData",
            async: false,
            dataType: "json",
            data: { "vJsonData": _fnMakeJson(objJsonData) },
            success: function (result) {
                if (JSON.parse(result).Result[0]["trxCode"] == "Y") {
                    ...
                    $(vThis).find(".BAT").text(JSON.parse(result).Inventory[0]["BATCH_ID"]);
                    vResult = true;
                }
        ...
        console.log("[Error - fnSetPlanData]" + err.message);
    }
}
    ...
    try {
        ...
        if (JSON.parse(vJsonData).Result[0]["trxCode"] == "Y") {
            ...
            $.each(vResult, function (i) {
                ...
                vHTML += "   	<td class=\"BAT\">" + _fnToNull(vResult[i]["BATCH_ID"]) + "</td> ";
