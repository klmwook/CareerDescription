...
//HU ID - 바코드 찍었을 경우 (Confirm창 띄우기)
$(document).on("keyup", "#Input_HuId", function (e) {
    //PDA에서는 KeyCode 0으로 뜸
    if (e.keyCode == 13) {
        _SuccessSound.play();
        if ($(this).val().length > 0) {
            layerPopup("#PutAway_Confirm");
        } else {
            _fnAlertMsg("HU ID를 입력 해주시기 바랍니다.");
        }
    }
});

//HU ID - Confirm 창 확인 버튼 이벤트 
$(document).on("click", "#PutAway_Confirm_Yes", function () {
    layerClose('#PutAway_Confirm');
    fnSetBoxItem();
});
...
function fnSetBoxItem() {
    try {
        ...
        objJsonData.BRANCH_CD = $("#Session_BRANCH_CD").val();
        objJsonData.USR_ID = $("#Session_USR_ID").val();
        objJsonData.RCP_MNGT_NO = $("#Select_Delivery_Note").find("option:selected").val();
        objJsonData.BOX_ID = $("#Input_HuId").val();

        $.ajax({
            type: "POST",
            url: "/T_PutAway/fnSetBoxItem",
            async: true,
            dataType: "json",
            data: { "vJsonData": _fnMakeJson(objJsonData) },
            success: function (result) {
                ...
                if (JSON.parse(result).Result[0]["trxCode"] == "Y") {
                    ...
                    //전역변수로 가지고 있는다.
                    _RCP_MNGT_NO = $("#Select_Delivery_Note").find("option:selected").val();
...
function fnSavePutAway() {
    try {
        if (fnSavePutAway_Validation()) {
            $.ajax({
                type: "POST",
                url: "/T_PutAway/fnSavePutAway",
                async: true,
                dataType: "json",
                data: { "vJsonData": _fnMakeJson(fnSetSaveParameter()) },
                success: function (result) {
