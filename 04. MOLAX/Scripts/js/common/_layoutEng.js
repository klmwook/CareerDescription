            ...
            success: function (result) {
                ...
                if (JSON.parse(result).Result[0]["trxCode"] == "Y") {
                    ...
                    if (JSON.parse(result).Table[0].APV_YN == "Y") {
                        ...
                        alert("Not approved. Please contact the person in charge.");
                    }
                    else if (JSON.parse(result).Table[0].APV_YN == "D") {
                        alert("Your subscription has been declined. Please check the reason for rejection in the email.");
                    }
                    else if (JSON.parse(result).Table[0].APV_YN == "S") {
                        alert("Your ID has been suspended. Please contact the person in charge.");
                    }
                }
                ...
                $("#loading-image").hide();
                console.log("We are unable to process the requested operation due to system conditions.");
                console.log(xhr);
                return;
            }, beforeSend: function () {
...
function _fnLogout() {
    $.ajax({
        ...
        success: function (result, status, xhr) {
            ...
            location.href = window.location.origin + "/?Language=EN";
        }
    });
}
/////////////////function MakeList/////////////////////
